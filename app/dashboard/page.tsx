...
export default async function DashboardPage() {
  ...
  let totalProducts, lowStock, allProducts;
  try {
    [totalProducts, lowStock, allProducts] = await Promise.all([
  ...
  // Calculate the number of products in stock, low stock, and out of stock
  const inStockCount = allProducts.filter((p) => Number(p.quantity) > 5).length;
  const lowStockCount = allProducts.filter((p) => Number(p.quantity) <= 5 && Number(p.quantity) > 0).length;
  const outOfStockCount = allProducts.filter((p) => Number(p.quantity) === 0).length;

  // Calculate the percentage of products in stock, low stock, and out of stock
  ...
  // Calculate the weekly products data
  const now = new Date();
  const weeklyProductsData = [];

  for (let i = 11; i >= 0; i--) {
    // Calculate the end of the week i weeks ago (23:59:59.999)
    const weekEnd = new Date(now);
    weekEnd.setDate(weekEnd.getDate() - i * 7);
    weekEnd.setHours(23, 59, 59, 999);

    // Start of that week is 6 days before (00:00:00.000)
    const weekStart = new Date(weekEnd);
    weekStart.setDate(weekEnd.getDate() - 6);
    weekStart.setHours(0, 0, 0, 0);

    const weekLabel = `${String(weekStart.getMonth() + 1).padStart(2, "0")}/${String(weekStart.getDate()).padStart(
      2,
      "0"
    )}`;
  ...
  try {
    ...
    recent = []; // Set to empty array if there's an error
  }
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            ...
            <div className="space-y-3">
              {recent.map((product, key) => {
                ...
                return (
                  <div key={key} className="flex items-center justify-between p-3 rounded-lg bg-gray-200">
