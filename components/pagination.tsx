...
export default function Pagination({ currentPage, totalPages, baseUrl, searchParams }: PaginationProps) {
  ...
  const getVisiblePages = () => {
    ...
    for (let i = Math.max(2, currentPage - delta); i <= Math.min(totalPages - 1, currentPage + delta); i++) {
      range.push(i);
      // If currentPage = 6, totalPages = 10, delta = 2 then range = [4, 5, 6, 7, 8]
    }
      ...
      {visiblePages.map((page, key) => {
        ...
        return (
          <Link
            ...
            className={`px-3 pyh-2 text-sm font-medium rounded-lg ${
              isCurrentPage
                ? "bg-blue-600 text-white"
                : "text-gray-700 hover:bg-gray100 bg-white border border-gray-300"
            }`}
          >
            {pageNumber}
          </Link>
